x=c(rnorm(500,500,100),rnorm(500,1000,200),rnorm(500,1500,400), rnorm(500,800,200))
y=c(abs(rnorm(500,50,25)),rnorm(500,200,50),rnorm(500,100,30), rnorm(500,150,20))
clases=as.factor(c(rep(1,500),rep(2,500),rep(3,500),rep(4,500)))
datos=data.frame(x,y,clases)
plot(x,y, data = datos, col = clases)

set.seed(1000)
testindex <- sample(1:nrow(datos),size=800)
testset <- datos[-testindex,] #prueba
trainset <- datos[testindex,] #entrenamiento

plot(x,y, data = trainset, col = clases)
...
nrow(subset(trainset,clases == 1))
nrow(subset(trainset,clases == 2))
nrow(subset(trainset,clases == 3))
nrow(subset(trainset,clases == 4))